CREATE OR REPLACE TABLE shopify-dw.scratch.modelled_internal_retail_mql_data
AS 
...
,opp_annual_total_revenue_verified__c) as opp_committed_total_gmv, COALESCE(ops_lead_source,'Unknown') as ops_lead_source
  ...
  AND (owner_role_at_creation__c LIKE '%AMER-SDR-LA-RETAIL%' OR ur.name LIKE '%AMER-SDR-MM-RETAIL%' OR ur.name LIKE '%AMER-SDR-SMB-RETAIL%')
...
-- , case when (((l.Owner_Role__c LIKE 'AMER-%N3%' AND extract (year from l.createddate) = 2024) OR c.name = 'N3_Lead_List_January_2024' OR c.name = 'Content_Syndication_Retail_January_N3')) then 'AMER' when (l.Owner_Role__c LIKE 'EMEA-%N3%') then 'EMEA' else 'Other' end as region
  ...
  WHERE l.isdeleted = false
    AND ((l.Owner_Role__c LIKE '%AMER-SDR-LA-RETAIL%' OR l.Owner_Role__c LIKE '%AMER-SDR-MM-RETAIL%' OR l.Owner_Role__c LIKE '%AMER-SDR-SMB-RETAIL%') AND extract (year from l.Marketing_Qualified_Lead_Date__c) = 2024)
)
...
where l.lead_id IS NOT NULL OR opp.sdr_role LIKE '%AMER-SDR-LA-RETAIL%' OR opp.sdr_role LIKE '%AMER-SDR-MM-RETAIL%' OR opp.sdr_role LIKE '%AMER-SDR-SMB-RETAIL%' OR opp.opp_owner_role LIKE '%AMER-SDR-LA-RETAIL%' OR opp.opp_owner_role LIKE '%AMER-SDR-MM-RETAIL%' OR opp.opp_owner_role LIKE '%AMER-SDR-SMB-RETAIL%'
)
...
case when Third_Party_Enriched_Revenue__c >= 200000000 then 200000000 else Third_Party_Enriched_Revenue__c end as Third_Party_Enriched_Revenue_max_200m
...
select 
  *,
  ...
  case when DATE(Marketing_Qualified_Lead_Date__c) > '2024-02-06' and final.lead_estimated_total_gmv is null 
    then Third_Party_Enriched_Revenue_max_200m else final.lead_estimated_total_gmv
  end as lead_estimated_offline_gmv,
