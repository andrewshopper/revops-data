CREATE OR REPLACE TABLE shopify-dw.scratch.modelled_n3_mql_emea_data
AS 
...
,opp_annual_total_revenue_verified__c) as opp_committed_total_gmv, COALESCE(ops_lead_source,'Unknown') as ops_lead_source
  ...
  AND (owner_role_at_creation__c LIKE 'EMEA-%N3%' OR ur.name LIKE 'EMEA-%N3%')
...
, leads as (
  ...
  WHERE l.isdeleted = false
    AND ((l.Owner_Role__c LIKE 'EMEA-%N3%' AND extract (year from l.createddate) = 2024))
)
...
where l.lead_id IS NOT NULL OR opp.sdr_role LIKE 'EMEA-%N3%' OR opp.opp_owner_role LIKE 'EMEA-%N3%'
)
