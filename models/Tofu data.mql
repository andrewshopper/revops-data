CREATE OR REPLACE TABLE shopify-dw.scratch.modelled_tofu_mql_data
AS 
...
,opp_annual_total_revenue_verified__c) as opp_committed_total_gmv, COALESCE(ops_lead_source,'Unknown') as ops_lead_source, team_segment
  FROM `shopify-dw.raw_salesforce_banff.opportunity` o 
  LEFT JOIN (SELECT DISTINCT opportunity_id, ops_lead_source, team_segment FROM `sdp-for-analysts-platform.rev_ops_prod.modelled_rad` WHERE dataset = 'Actuals' AND full_date >= '2023-07-01') fun ON o.id = fun.opportunity_id
...
, leads as (
  ...
  AND DATE(l.createddate) >= '2023-07-01'
...
case when lower(lead_owner) like '%forecasting%role%' and account_id is not null and Annual_Offline_Revenue__c is null then 'MM'
when lower(lead_owner) like '%forecasting%role%' and account_id is null and Annual_Offline_Revenue__c is not null then 'Retail'
when lower(lead_owner) like '%forecasting%role%' and account_id is null and Annual_Offline_Revenue__c is null then 'D2C MM'
end as segment
...
select 
  *,
  ...
  CASE WHEN Marketing_Qualified_Lead_Date__c IS NOT NULL THEN 1 ELSE 0 END is_mql, 
  ...
  CASE WHEN lead_id IS NULL THEN 'Outbound' 
    ...
    WHEN Marketing_Qualified_Lead_Date__c IS NULL THEN 'Lead not qualified' 
    ELSE 'MQL'
  END as sql_source, 
  CASE WHEN opp_stage_name NOT IN ('Pre-Qualified','Closed Lost') THEN 1 ELSE 0 END as is_sal  
from final
-- WHERE COALESCE(account_id, '') NOT IN ('0018V00002d2BY2QAM') -- excludes Retail account
